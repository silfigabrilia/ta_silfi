...
if (isset($_POST['save'])) {
    ...
    if (empty($error)) {
        if ($action == 'add') {
            if (mysqli_num_rows(mysqli_query($con, "select * from gizi where nama='" . $nama . "'")) > 0) {
                $error = 'nama sudah ada';
            } else {
                $q = "insert into gizi(nama,keterangan) values ('" . $nama . "','" . $keterangan . "')";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
        if ($action == 'edit') {
            $q = mysqli_query($con, "select * from gizi where id_gizi='" . $id . "'");
            $r = mysqli_fetch_array($q);
            $nama_tmp = $r['nama'];
            if (mysqli_num_rows(mysqli_query($con, "select * from gizi where nama='" . $nama . "' and nama<>'" . $nama_tmp . "'")) > 0) {
                $error = 'nama sudah ada';
            } else {
                $q = "update gizi set nama='" . $nama . "',keterangan='" . $keterangan . "' where id_gizi='" . $id . "'";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
    ...
    if ($action == 'delete') {
        $id = $_GET['id'];
        mysqli_query($con, "delete from gizi where id_gizi='" . $id . "'");
        exit("<script>location.href='" . $link_data . "';</script>");
    }
}
?>
