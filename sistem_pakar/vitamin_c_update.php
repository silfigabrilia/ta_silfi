<?php
$link_data = '?page=vitamin_c';
$link_update = '?page=update_vitamin_c';
...
if (isset($_POST['save'])) {
    ...
    if (empty($error)) {
        if ($action == 'add') {
            if (mysqli_num_rows(mysqli_query($con, "select * from vitamin_c where nama='" . $nama . "'")) > 0) {
                $error = 'nama sudah ada';
            } else {
                $q = "insert into vitamin_c(nama,keterangan) values ('" . $nama . "','" . $keterangan . "')";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
        if ($action == 'edit') {
            $q = mysqli_query($con, "select * from vitamin_c where id_vitamin_c='" . $id . "'");
            $r = mysqli_fetch_array($q);
            $nama_tmp = $r['nama'];
            if (mysqli_num_rows(mysqli_query($con, "select * from vitamin_c where nama='" . $nama . "' and nama<>'" . $nama_tmp . "'")) > 0) {
                $error = 'nama sudah ada';
            } else {
                $q = "update vitamin_c set nama='" . $nama . "',keterangan='" . $keterangan . "' where id_vitamin_c='" . $id . "'";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
    ...
    if ($action == 'delete') {
        $id = $_GET['id'];
        mysqli_query($con, "delete from vitamin_c where id_vitamin_c='" . $id . "'");
        exit("<script>location.href='" . $link_data . "';</script>");
    }
}
?>
<div class="box box-success">
    <div class="box-header with-border">
        <h3 class="box-title">Pengetahuan Vitamin C</h3>
    </div>
