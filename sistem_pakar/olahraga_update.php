<?php
$link_data = '?page=olahraga';
$link_update = '?page=update_olahraga';
...
if (isset($_POST['save'])) {
    ...
    if (empty($error)) {
        if ($action == 'add') {
            if (mysqli_num_rows(mysqli_query($con, "select * from olahraga where nama='" . $nama . "'")) > 0) {
                $error = 'nama sudah ada';
            } else {
                $q = "insert into olahraga(nama,keterangan) values ('" . $nama . "','" . $keterangan . "')";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
        if ($action == 'edit') {
            $q = mysqli_query($con, "select * from olahraga where id_olahraga='" . $id . "'");
            $r = mysqli_fetch_array($q);
            $nama_tmp = $r['nama'];
            if (mysqli_num_rows(mysqli_query($con, "select * from olahraga where nama='" . $nama . "' and nama<>'" . $nama_tmp . "'")) > 0) {
                $error = 'nama sudah ada';
            } else {
                $q = "update olahraga set nama='" . $nama . "',keterangan='" . $keterangan . "' where id_olahraga='" . $id . "'";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
    ...
    if ($action == 'delete') {
        $id = $_GET['id'];
        mysqli_query($con, "delete from olahraga where id_olahraga='" . $id . "'");
        exit("<script>location.href='" . $link_data . "';</script>");
    }
}
?>
<div class="box box-success">
    <div class="box-header with-border">
        <h3 class="box-title">Pengetahuan Olahraga</h3>
    </div>
