...
public class Vitamin_E extends AppCompatActivity {

    private ProgressDialog pDialog;
    private static String URL_VitaminE = "https://sistempakarta.000webhostapp.com/android/get_vitamin_e.php";
    private ListView lv;
    ...
    private void displayLoader() {
        pDialog = new ProgressDialog(Vitamin_E.this);
    ...
    private void getVitaminE() {
        displayLoader();

        RequestQueue queue = Volley.newRequestQueue(Vitamin_E.this);
        JsonObjectRequest jsArrayRequest = new JsonObjectRequest(Request.Method.GET, URL_VitaminE, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                pDialog.dismiss();
                try {
                    if (response.getInt("status") == 0) {
                        JSONArray jsonArray = response.getJSONArray("vitamin_e");
